...
TP_ATR_MULT = 0.4
SL_ATR_MULT = 0.4
INTERVAL_ATR_MULT = 0.4
RELIST_INTERVAL_ATR_MULT = 0.5
...
class MM002_OrderMakerStrategy(GenericStrategy):
    ...
    def is_position_in_loss(self, is_long, avg_entry_price, ticker_last_price):
        if is_long:
            return ticker_last_price < avg_entry_price
        else:
            return ticker_last_price > avg_entry_price

    def get_tp_price(self, is_long, instrument, avg_entry_price, ticker_last_price):
        is_loss_position = self.is_position_in_loss(is_long, avg_entry_price, ticker_last_price)
        take_profit_pct = TP_ATR_MULT * self.curr_market_snapshot.atr_pct
        base_price = avg_entry_price if is_loss_position else ticker_last_price
        if is_long:
            price = base_price * (1 + take_profit_pct)
        else:
            price = base_price * (1 - take_profit_pct)
        price = mm_math.toNearest(price, instrument['tickSize'])
        return price

    def get_sl_price(self, is_long, instrument, avg_entry_price, ticker_last_price):
        is_loss_position = self.is_position_in_loss(is_long, avg_entry_price, ticker_last_price)
        stop_loss_pct = SL_ATR_MULT * self.curr_market_snapshot.atr_pct
        if is_long:
            if not is_loss_position:
                price = ticker_last_price * (1 - stop_loss_pct)
            else:
                price = avg_entry_price * (1 - stop_loss_pct)
                if price >= ticker_last_price:
                    price = ticker_last_price * (1 - stop_loss_pct)
        else:
            if not is_loss_position:
                price = ticker_last_price * (1 + stop_loss_pct)
    ...
    def get_price(self, index, instrument, ticker_last_price):
        base_price = ticker_last_price
        price = base_price * (1 + index * INTERVAL_ATR_MULT * self.curr_market_snapshot.atr_pct)
        price = mm_math.toNearest(price, instrument['tickSize'])
        return price
    ...
    def prepare_tp_order(self, is_long, quantity):
        ...
        price = self.get_tp_price(is_long, instrument, avg_entry_price, ticker_last_price)
    ...
    def prepare_sl_order(self, is_long, quantity):
        ...
        price = self.get_sl_price(is_long, instrument, avg_entry_price, ticker_last_price)
    ...
    def validate_orders(self, orders, instrument, running_qty, avgEntryPrice, ticker_last_price, quoting_side):
        relist_interval_pct = RELIST_INTERVAL_ATR_MULT * self.curr_market_snapshot.atr_pct
        ...
        if running_qty != 0:
            if running_qty > 0:
                ...
                tp_desired_price = self.get_tp_price(True, instrument, avgEntryPrice, ticker_last_price)
                if self.is_price_diff_exceeded_value(tp_order["price"], tp_desired_price, relist_interval_pct):
                    return False

                sl_desired_price = self.get_sl_price(True, instrument, avgEntryPrice, ticker_last_price)
                if self.is_price_diff_exceeded_value(sl_order["stopPx"], sl_desired_price, relist_interval_pct):
                    return False
            else:
                ...
                tp_desired_price = self.get_tp_price(False, instrument, avgEntryPrice, ticker_last_price)
                if self.is_price_diff_exceeded_value(tp_order["price"], tp_desired_price, relist_interval_pct):
                    return False

                sl_desired_price = self.get_sl_price(False, instrument, avgEntryPrice, ticker_last_price)
                if self.is_price_diff_exceeded_value(sl_order["stopPx"], sl_desired_price, relist_interval_pct):
                    return False
        else:
            quoting_side = settings.QUOTING_SIDE
